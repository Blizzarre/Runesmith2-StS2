...
// Code taken from https://github.com/lamali292/Downfall/blob/main/DownfallCode/Data/RunMetricsUploader.cs
...
public sealed class MetricsUploaderConfig
{
    ...
    public bool RejectForeignContent { get; init; } = false;
    ...
    /// </summary>
    public Func<string, string, string, bool, string>? WrapPayload { get; init; }
...
public static class MetricsPredicates
{
    ...
    {
        return c => c.GetType().Assembly == assembly;
    }
}
...
public sealed class RunMetricsUploader<TPayload>
{
    ...
    public void Upload(SerializableRun run, bool isVictory, ulong localPlayerId)
    {
        ...
        _ = SendToServer(json, HasForeignContent(run));
    }
    ...
    private bool ShouldUpload(SerializableRun run, ulong localPlayerId)
    {
        ...
        if (!RunesmithConfig.UploadMetrics)
        {
            log?.Info($"[{mod}] Skipping metrics upload, user has upload data unset.");
            return false;
        }
    ...
    private async Task SendToServer(string dataJson, bool hasForeign = false)
    {
        var log = _config.Logger;
        var mod = _config.ModName;
        try
        {
            ...
            var body = _config.WrapPayload is { } wrap
                ? wrap(dataJson, version, mod, hasForeign)
    ...
    {
        return $"{{\"mod_version\":{JsonSerializer.Serialize(version)},\"data\":{dataJson}}}";
    }
